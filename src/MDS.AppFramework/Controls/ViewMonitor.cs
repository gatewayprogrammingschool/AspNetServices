...
public record ViewMonitor(IViewWorkflow View, ILogger Logger) : IViewMonitor
{
    ...
    public async Task StartAsync(HttpContext context, CancellationToken token = default)
    {
        ...
        try
        {
            Logger.LogInformation(
                $"{GetType().Name}: Starting {View.GetType().Name}::{View.ViewKey}"
            );
        ...
        catch (Exception ex)
        {
            Logger.LogError(
                ex,
                $"{GetType().Name}: Handled {View.GetType().Name}::{View.ViewKey}{Environment.NewLine}{ex}"
            );
        ...
        {
            Logger.LogInformation(
                $"{GetType().Name}: Processed {View.GetType().Name}::{View.ViewKey}"
            );

            if (!View.IsCompleted)
            {
                ae.InnerExceptions.Append(
                    new ApplicationException("View did not complete processing.")
                );
            }
        }
    ...
    public async Task StopAsync(CancellationToken token)
    {
        ...
        {
            Logger.LogInformation(
                $"{GetType().Name}: Stopping {View.GetType().Name}::{View.ViewKey}"
            );
            _tokenSource.Cancel();
        ...
        {
            Logger.LogInformation(
                $"{GetType().Name}: Stopped {View.GetType().Name}::{View.ViewKey}"
            );
    ...
    public event Func<IViewWorkflow, AggregateException, CancellationToken, Task>?
        ViewNotCompletedAsync;
